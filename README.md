...
- **stellar-core** 28.0.1
- **horizon** 28.0.1
- **stellar-rpc** 28.0.1 (Soroban RPC, disabled by default)
...
```shell
$ docker run --rm -it -p "31401:8000" -v "/path/to/data:/opt/stellar" --name pi-node pinetwork/pi-node-docker:mainnet-relay-v1.0-p28.0.1 --mainnetrelay
```
...
$ docker run -it --rm \
    ...
    -p "31403:1570" \
    --name pi-node \
    pinetwork/pi-node-docker:mainnet-relay-v1.0-p28.0.1 --mainnetrelay
```
...
services:
  mainnet:
    image: pinetwork/pi-node-docker:mainnet-relay-v1.0-p28.0.1
...
This builds the image as `pinetwork/pi-node-docker:mainnet-relay-v1.0-p28.0.1`.
