...
```shell
$ docker run --rm -it -p "31401:8000" -v "/path/to/data:/opt/stellar" --name pi-node pinetwork/pi-node-docker:mainnet-relay-v1.1-p23.0.1 --mainnetrelay
```
...
$ docker run -it --rm \
    ...
    -p "31403:1570" \
    --name pi-node \
    pinetwork/pi-node-docker:mainnet-relay-v1.1-p23.0.1 --mainnetrelay
```
...
services:
  mainnet:
    image: pinetwork/pi-node-docker:mainnet-relay-v1.1-p23.0.1
...
This builds the image as `pinetwork/pi-node-docker:mainnet-relay-v1.1-p23.0.1`.
