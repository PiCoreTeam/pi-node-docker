#!/usr/bin/env bash
#
# Migration 008: refresh bin/start scripts on volume from image defaults.
#
...
for service in core horizon stellar-rpc; do
    ...
    if [[ ! -f "$src" ]]; then
        echo "[008] no source for ${service} (${src}), skipping"
        continue
    fi
    if [[ ! -f "$dst" ]]; then
        echo "[008] no destination for ${service} (${dst}), skipping"
        continue
    fi

    cp "$dst" "${BACKUP_DIR}/${service}-bin-start.008.bak"
    cp "$src" "$dst"
    chmod +x "$dst"
    echo "[008] refreshed ${dst}"
done
