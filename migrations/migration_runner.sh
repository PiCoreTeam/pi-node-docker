...
log() {
    ...
    case "$level" in
        OK)    echo "${timestamp} ${LOG_PREFIX} [OK]   ${message}" ;;
        SKIP)  echo "${timestamp} ${LOG_PREFIX} [SKIP] ${message}" ;;
        ERROR) echo "${timestamp} ${LOG_PREFIX} [ERROR] ${message}" >&2 ;;
        INFO)  echo "${timestamp} ${LOG_PREFIX} [INFO] ${message}" ;;
...
main() {
    ...
    log INFO "=========================================="
    log INFO "Total: ${total_count}  Executed: ${executed_count}  Skipped: ${skipped_count}"
