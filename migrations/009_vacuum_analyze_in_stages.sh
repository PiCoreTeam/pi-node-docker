...
if ! command -v pg_isready >/dev/null 2>&1; then
    echo "[009] pg_isready not found; skipping"
    exit 0
fi

if ! pg_isready -q; then
    echo "[009] postgres is not running; skipping (run vacuumdb manually later)"
    exit 0
fi

echo "[009] running vacuumdb --all --analyze-in-stages -j ${JOBS} ..."
sudo -u postgres vacuumdb --all --analyze-in-stages -j "${JOBS}"
echo "[009] vacuumdb completed"
