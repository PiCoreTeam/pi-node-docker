#!/usr/bin/env bash
#
# Migration 006: refresh bin/start scripts on volume from image defaults.
#
...
for service in core horizon stellar-rpc; do
    ...
    if [[ ! -f "$src" ]]; then
        echo "[006] no source for ${service} (${src}), skipping"
        continue
    fi
    if [[ ! -f "$dst" ]]; then
        echo "[006] no destination for ${service} (${dst}), skipping"
        continue
    fi

    cp "$dst" "${BACKUP_DIR}/${service}-bin-start.006.bak"
    cp "$src" "$dst"
    chmod +x "$dst"
    echo "[006] refreshed ${dst}"
done
